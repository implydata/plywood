var { expect } = require("chai");

var plywood = require('../../build/plywood');
var { Expression, Action, Dataset, $, ply, r } = plywood;

describe("reference check", function() {
  var context = {
    diamonds: Dataset.fromJS([
      { color: 'A', cut: 'great', carat: 1.1, price: 300 }
    ])
  };

  describe("works as expected", function() {
    it("works when there are no free references", function() {
      var ex = ply()
        .apply('num', 5)
        .apply(
          'subData',
          ply()
            .apply('x', '$num + 1')
            .apply('y', '$x * 2')
        );

      ex = ex.referenceCheck({});
      return expect(ex.getFreeReferences()).to.deep.equal([]);
    });

    it("works in a basic case", function() {
      var ex = Expression.parse('$x + $y * $z + $data.sum($revenue)');

      return expect(ex.getFreeReferences()).to.deep.equal(['data', 'x', 'y', 'z']);
    });

    it("works in a actions case", function() {
      var ex = ply()
        .apply('num', 5)
        .apply(
          'subData',
          ply()
            .apply('x', '$num + 1')
            .apply('y', '$x * 2')
            .apply('z', '$diamonds.sum($price)')
        );

      ex = ex.referenceCheck(context);
      expect(ex.getFreeReferences()).to.deep.equal(['diamonds']);

      return expect(ex.actions[1].getFreeReferences()).to.deep.equal(['^diamonds', 'num']);
    });

    return it("works in a consecutive actions case", function() {
      var ex = ply()
        .apply('one', 1)
        .apply('two', '$one + 1')
        .apply('three', '$two + 1')
        .apply('four', '$three + 1');


      ex = ex.referenceCheck({});
      return expect(ex.getFreeReferences()).to.deep.equal([]);
    });
  });


  return describe('checks action decencies', function() {
    var ex = ply()
      .apply('two', '$one + 1')
      .apply('three', '$two + 1')
      .apply('four', '$three + 1');

    it("finds something that is needed", function() {
      return expect(Action.actionsDependOn(ex.actions, 'one')).to.be.true;
    });

    it("knows what is not needed because it is not referred to", function() {
      return expect(Action.actionsDependOn(ex.actions, 'kaka')).to.be.false;
    });

    return it("knows what is not needed because it is shadowed", function() {
      return expect(Action.actionsDependOn(ex.actions, 'three')).to.be.false;
    });
  });
});
